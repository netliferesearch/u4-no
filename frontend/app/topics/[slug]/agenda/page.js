          ...
          <section className="o-wrapper-medium">
            <div className="c-article__row">
              <div className="content c-article__col">
                <LongformArticle>
                  <LongformArticleContent content={agenda} title={title} />
                </LongformArticle>
              </div>
...
export async function generateMetadata({ params }) {
  ...
  return getMetadata({
    title: firstTitleIn( agenda ) || `Basic guide - ${title}`,
    description: firstParagraphIn( agenda),
...
export async function generateStaticParams() {
  const sanityQuery = groq`*[_type == 'topics' && defined(agenda)]{ "slug": slug.current } | order(_updatedAt desc) [0..1000]`;
