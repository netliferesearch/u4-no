export const footerMenu = [
  { id: 'topics', label: 'Research topics', slug: '/topics' },
  { id: 'blog', label: 'Blog', slug: '/blog' },
  { id: 'courses', label: 'Online courses', slug: '/online-courses' },
  { id: 'privacy-policy', label: 'Privacy policy', slug: '/privacy-policy' },
  { id: 'about-u4', label: 'Vision & Strategy', slug: '/about-u4' },
  { id: 'the-team', label: 'People', slug: '/the-team' },
];

export const footerText = {
  url: 'https://www.cmi.no/',
  description:
    'The U4 Anti-Corruption Resource Centre works to reduce the harmful impact of corruption on society. U4 is a permanent centre at Chr. Michelsen Institute in Norway.',
};
